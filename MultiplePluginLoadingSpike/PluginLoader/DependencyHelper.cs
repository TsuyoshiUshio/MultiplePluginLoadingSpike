...
namespace MultiplePluginLoadingSpike.PluginLoader
{
    public static class DependencyHelper
    {
        ...
        {
            return GetResourceFileContents(Path.Combine("metadata", "runtimes.json"));
        }

        private static string GetResourceFileContents(string fileName)
        {
            var assembly = typeof(DependencyHelper).Assembly;
            return File.ReadAllText(Path.Combine(Path.GetDirectoryName(assembly.FullName), fileName));
            // TODO ask Fabio the intention
            //var assembly = typeof(DependencyHelper).Assembly;
