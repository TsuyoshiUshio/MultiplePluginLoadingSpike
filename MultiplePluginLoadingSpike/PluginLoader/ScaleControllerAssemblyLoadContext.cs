...
namespace MultiplePluginLoadingSpike.PluginLoader
{
    ...
    /// </summary>
    public partial class ScaleControllerAssemblyLoadContext : AssemblyLoadContext
    {
        ...
        private static Lazy<ScaleControllerAssemblyLoadContext> _defaultContext = new(() => CreateSharedContext(ResolveFunctionBaseProbingPath()), true);
        ...
        public ScaleControllerAssemblyLoadContext(string basePath)
        {
            if (basePath == null)
            {
        ...
        public static ScaleControllerAssemblyLoadContext Shared => _defaultContext.Value;

        internal static void ResetSharedContext(string baseProbingPath = null)
        {
            baseProbingPath ??= ResolveFunctionBaseProbingPath();

            _defaultContext = new Lazy<ScaleControllerAssemblyLoadContext>(() => CreateSharedContext(baseProbingPath), true);
        ...
        {
            string depsFilePath = GetDepsPath(basePath);
            // 
        ...
        private static string GetDepsPath(string basePath)
        {
            var pluginFiles = new DirectoryInfo(basePath).EnumerateFiles().Where(f => f.Name.EndsWith("deps.json"));
            if (pluginFiles.Any())
            {
                return pluginFiles.FirstOrDefault().FullName;
            }
            throw new ArgumentException($"Can not find deps.json on the {basePath} directory");
        } 
        ...
        private static ScaleControllerAssemblyLoadContext CreateSharedContext(string baseProbingPath)
        {
            var sharedContext = new ScaleControllerAssemblyLoadContext(baseProbingPath);
        ...
        // Represent the bin directory of the MultiplePluginLoadingSpike 
